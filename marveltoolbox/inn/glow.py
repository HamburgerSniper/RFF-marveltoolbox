...
parser.add_argument('--batch_size_init', type=int, default=256,
...
parser.add_argument('--n_epochs_warmup', type=int, default=2,
...
parser.add_argument('--checkpoint_grads', action='store_true', default=False,
...
def fetch_dataloader(args, train=True, data_dependent_init=False):
    ...
    return DataLoader(dataset, batch_size=batch_size, shuffle=(sampler is None), drop_last=True, sampler=sampler,
                      **kwargs)
...
def get_manipulators(zs, attrs):
    ...
    dzs = 1.6 * torch.stack([compute_dz(zs, attrs, i) for i in range(attrs.shape[1])],
                            dim=0)  # compute dz for each attribute official code multiplies by 1.6 scalar here
    print('Completed.')
...
if __name__ == '__main__':
    ...
    args.output_dir = os.path.dirname(args.restore_file) if args.restore_file else os.path.join(args.output_dir,
                                                                                                time.strftime(
                                                                                                    '%Y-%m-%d_%H-%M-%S',
                                                                                                    time.gmtime()))
    ...
    if args.visualize:
        ...
        dec_x = visualize(model, args, args.vis_attrs, args.vis_alphas,
                          args.vis_img)  # output (n_attr, n_alpha, 3, H, W)
        ...
        save_image(dec_x.view(-1, *args.input_dims), os.path.join(args.output_dir, filename + '.png'),
                   nrow=dec_x.shape[1])
